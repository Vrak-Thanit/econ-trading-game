...
async function cloneGameFromTemplate(newGameId: string) {
  ...
  const baseItemPrices = normalizeItemPrices(
    t?.itemBasePrices ?? t?.itemPrices ?? {}
  );

  const baseItemCur =
    Object.keys(baseItemPrices).length > 0
      ? baseItemPrices
      : normalizeItemPrices({});
...
export default function TeacherControls({ gameId }: { gameId: string }) {
  ...
  const itemPriceRows = useMemo(() => {
    ...
    const names = Array.from(
      new Set([...Object.keys(base), ...Object.keys(cur)])
    ).sort();
  ...
  async function closeRoundNoSettle() {
    ...
    try {
      ...
      if (String(g.phase ?? "") !== "LIVE") {
        throw new Error("Close Round is only allowed during LIVE phase");
      }
  ...
  async function endRoundSettlement() {
    ...
    try {
      ...
      if (g.phase !== "LIVE") {
        throw new Error("End Round only allowed in LIVE phase");
      }
      ...
      for (const o of latestOrders) {
        orderedByTeam[o.teamId] =
          (orderedByTeam[o.teamId] ?? 0) + Number(o.plannedQty ?? 0);
      }
      ...
      teamsSnap.forEach((t) => {
        ...
        for (const s of SHAPES) {
          deliveredTotal += Number(deliveredMap?.[s] ?? 0);
        }
        ...
        const newReliability = Math.max(
          0,
          Math.min(100, prevReliability - drop)
        );
        ...
        if (orderedTotal > 0 || deliveredTotal > 0 || debt > 0) {
          summaryLines.push(
            `${teamId}: ordered ${orderedTotal}, delivered ${deliveredTotal}, shortfall ${shortfall}, fine ${fine}, interest ${interestDue} (${(
              ...
              preview.inGrace ? ", grace" : ""
            }), debt ${debt}→${newDebt}, reliability ${prevReliability}→${newReliability}`
          );
        }
      });
  ...
  async function resetValuesOnly() {
    ...
    try {
      ...
      if (!g.shapeBasePrices) {
        throw new Error("Missing shapeBasePrices in game doc.");
      }

      if (!g.itemBasePrices) {
        throw new Error("Missing itemBasePrices in game doc.");
      }

      if (!g.teamBaseState) {
        throw new Error("Missing teamBaseState in game doc.");
      }

      await deleteCollectionInBatches(collection(db, "games", gameId, "orders"));
      await deleteCollectionInBatches(collection(db, "games", gameId, "sales"));
      await deleteCollectionInBatches(collection(db, "games", gameId, "trades"));
      await deleteCollectionInBatches(collection(db, "games", gameId, "rentals"));
      await deleteCollectionInBatches(
        collection(db, "games", gameId, "loanRequests")
      );
      await deleteCollectionInBatches(
        collection(db, "games", gameId, "auctions")
      );
      await deleteCollectionInBatches(collection(db, "games", gameId, "logs"));
      ...
      batch.set(logRef, {
        type: "FULL_CLASSROOM_RESET",
        createdAt: serverTimestamp(),
        message:
          "Teacher reset game values and cleared orders, sales, trades, rentals, loans, auctions, and logs.",
      });
      ...
      setDirty(false);
      setMode("NORMAL");
      setSaveMsg("Full classroom reset complete ✅ Rentals cleared too.");
      alert("Full classroom reset complete ✅ Rentals cleared too.");
    } catch (e: any) {
      setErr(e?.message ?? String(e));
    }
  }
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <input
            ...
            value={jumpTo}
            onChange={(e) =>
              setJumpTo(Math.max(0, Math.floor(Number(e.target.value || 0))))
            }
            ...
            onClick={async () => {
              ...
              try {
                ...
                if (String(g.phase ?? "") !== "INTERVAL") {
                  throw new Error(
                    "Soft Jump is only allowed in INTERVAL phase (to avoid breaking LIVE round)."
                  );
                }
      ...
      >
        <label style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <select
            value={mode}
            onChange={(e) => setMode(e.target.value as EconomyMode)}
          >
        ...
        <button
          ...
          onClick={async () => {
            ...
            try {
              ...
              const lastDemandRaw = (g.currentRound?.demand ??
                {}) as Record<string, number>;

              const lastDemand: any = {};

              for (const s of SHAPES) {
                lastDemand[s] = Number(lastDemandRaw[s] ?? 0);
              }
              ...
              setDirty(true);
              setSuggestMsg(
                "Suggested demand generated ✅ (you can edit before saving)"
              );
            } catch (e: any) {
              setErr(e?.message ?? String(e));
            }
      ...
      >
        ...
        <div style={{ opacity: 0.85, marginBottom: 10 }}>
          Economy index (cash vs baseline):{" "}
          <b>{num(game.economyIncomeIndex ?? 1).toFixed(2)}</b> | Item
          multiplier: <b>{num(game.economyItemMult ?? 1).toFixed(2)}</b>
        </div>
      ...
      >
        ...
        <button
          ...
          onClick={async () => {
            ...
            try {
              ...
              if (g.phase !== "INTERVAL") {
                throw new Error("Start Round only allowed in INTERVAL phase");
              }
              ...
              const baseRaw = (g.shapeBasePrices ?? {}) as Record<
                string,
                number
              >;
              ...
              const prevPricesRaw = (g.currentRound?.prices ??
                {}) as Record<string, number>;
              ...
              const prevItemPrices = normalizeItemPrices(
                g.itemPrices ?? g.itemBasePrices ?? {}
              );

              const baseState = (g.teamBaseState ?? {}) as Record<
                string,
                { cash?: number }
              >;

              const baselineTotalCash = Object.values(baseState).reduce(
              ...
              const { nextItemPrices, incomeIndex, mult } =
                computeNextItemPrices({
                  baseItemPrices,
                  prevItemPrices,
                  totalCashNow,
                  baselineTotalCash,
              ...
              setSaveMsg(
                "Round started ✅ shape + item prices updated (±20% cap, smooth 0.40)"
              );
        ...
        <button
          ...
          onClick={resetValuesOnly}
        >
          Full Reset Values + Records
        </button>

        <button
          style={{ padding: "8px 12px", border: "1px solid orange" }}
        ...
        <button
          style={{ padding: "8px 12px", border: "1px solid #f43f5e" }}
      ...
      >
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                ...
                setSaveMsg(
                  `Created ✅ ${res.newGameId} | Teams copied: ${res.teamsCopied}`
                );
        ...
        <div style={{ marginTop: 8, opacity: 0.8 }}>
          Students will join using the new <b>gameId</b>. Orders/trades/logs/etc
          start fresh because subcollections aren&apos;t copied.
        </div>
      </div>
      ...
      {saveMsg && (
        <div style={{ marginTop: 8, color: "lightgreen" }}>{saveMsg}</div>
      )}

      {err && <div style={{ marginTop: 8, color: "crimson" }}>{err}</div>}

      <small style={{ opacity: 0.8 }}>
        Start Round updates shape prices + item prices (smooth 0.40, capped
        ±20%). End Round applies fines + reliability + interest.
      </small>
    </div>
  );
}